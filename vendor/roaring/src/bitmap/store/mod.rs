...
impl Store {
    pub(crate) fn allocation_size(&self) -> usize {
        match self {
            Array(array) => array.capacity() * core::mem::size_of::<u16>(),
            Bitmap(_) => BITMAP_BYTES,
            Run(runs) => runs.allocation_size(),
        }
    }

    pub(crate) fn point_allocation_bound(&self, inserting: bool) -> usize {
        match self {
            Array(array) if inserting => {
                let growth = if array.len() as usize == array.capacity() {
                    array.capacity().saturating_mul(2).max(4) * core::mem::size_of::<u16>()
                } else { 0 };
                growth + if array.len() >= ARRAY_LIMIT { BITMAP_BYTES } else { 0 }
            }
            Bitmap(bits) if !inserting && bits.len() <= ARRAY_LIMIT + 1 => BITMAP_BYTES,
            Run(runs) => runs.point_allocation_bound(),
            _ => 0,
        }
    }
