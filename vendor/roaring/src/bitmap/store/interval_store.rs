...
impl IntervalStore {
    ...
    pub(crate) fn allocation_size(&self) -> usize {
        self.0.capacity() * core::mem::size_of::<Interval>()
    }

    pub(crate) fn point_allocation_bound(&self) -> usize {
        if self.0.len() == self.0.capacity() {
            self.0.capacity().saturating_mul(2).max(4) * core::mem::size_of::<Interval>()
        } else { 0 }
    }
