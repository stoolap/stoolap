...
impl RoaringBitmap {
    ...
    /// Physical capacity local to the container containing `value`:
    /// `(container directory bytes, target store bytes, point mutation bound)`.
    /// The final value bounds additional allocation during one insert/remove,
    /// including a possible array/bitmap conversion. It is conservative and
    /// does not imply that a duplicate insert or absent remove will allocate.
    /// No bitmap/container scan is performed. Keys outside this container have
    /// unchanged allocation after a point mutation, so callers can apply deltas.
    pub fn allocation_for_value(&self, value: u32, inserting: bool) -> (usize, usize, usize) {
        let (key, _) = util::split(value);
        let directory = self.containers.capacity() * core::mem::size_of::<Container>();
        match self.containers.binary_search_by_key(&key, |c| c.key) {
            Ok(slot) => {
                let store = &self.containers[slot].store;
                (directory, store.allocation_size(), store.point_allocation_bound(inserting))
            }
            Err(_) => {
                let bound = if inserting {
                    let growth = if self.containers.len() == self.containers.capacity() {
                        self.containers.capacity().saturating_mul(2).max(4) * core::mem::size_of::<Container>()
                    } else { 0 };
                    growth + 4 * core::mem::size_of::<u16>()
                } else { 0 };
                (directory, 0, bound)
            }
        }
