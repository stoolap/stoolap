    ...
    fn from_column(column: &'a ColumnData, index: usize) -> Self {
        ...
        match column {
            ...
            ColumnData::TimestampNanos { values, .. } => {
                let nanos = values[index];
                // Every i64 nanosecond offset lies inside chrono's range.
                Self::Timestamp(
                    DateTime::from_timestamp(
                        nanos.div_euclid(1_000_000_000),
                        nanos.rem_euclid(1_000_000_000) as u32,
                    )
                    .expect("i64 nanosecond timestamp"),
                )
            }
...
impl Predicate<'_> {
    ...
    fn matches_cold(
        ...
        if let Some(predicate) = dictionary {
            let (column, offset) = columns.columns[predicate.physical].get().unwrap().data();
    ...
    fn bind(&self, logical: usize) -> Result<()> {
        ...
        let column = if source.should_use_group_cache() {
            Column::Group(
                source
                    .compressed_store()
                    .expect("compressed group source")
        ...
        // Slots are written once. Borrowed aggregate inputs can retain their
        // column reference while other required columns are bound lazily.
    ...
    fn get(&self, logical: usize, local: usize) -> Cell<'_> {
        match &self.segment.mapping.sources[logical] {
            ...
            ColSource::Volume(physical) => {
                let (column, offset) = self.columns[*physical]
                    .get()
                    .expect("required aggregate column is bound")
                    .data();
...
pub(crate) fn filtered(
    ...
    Ok(
        run(schema, view, cold, &[], aggregates, Some(expression))?.map(|mut results| {
            results
                .pop()
                .expect("global aggregate always has a group")
                .aggregate_values
        }),
    )
}
    ...
    if let Some((generation, pending)) = cold {
        ...
        for &id in &generation.segment_ids_newest_first {
                ...
                .enumerate()
            {
                ...
                if loaded.is_none() {
                    loaded = Some(generation.load_segment(id)?);
                }
                let columns = GroupColumns::new(loaded.as_ref().unwrap(), group, ids.len());
                for predicate in &predicates_bound {
                ...
                'rows: loop {
                    ...
                    if bound_aggregates.is_none() {
                        for &column in &needed {
                            columns.bind(column)?;
                        }
                        bound_aggregates = Some(
                            aggregates
                                .iter()
                                .map(|&(operation, column)| columns.aggregate(operation, column))
                                .collect(),
                        );
                    }
                    ...
                    if group_by.len() == 1 {
                        ...
                        {
                            let (data, offset) = columns.columns[physical].get().unwrap().data();
                    ...
                    for (accumulator, aggregate) in groups.accumulators[index]
                        .iter_mut()
                        .zip(bound_aggregates.as_ref().unwrap())
                    {
