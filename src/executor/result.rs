...
pub struct ExecutorResult {
    ...
    insert_id: i64,
    column_memory: HotMetadataCharge,
}
...
impl ExecutorResult {
    ...
    pub fn new(columns: Vec<String>, rows: RowVec) -> Self {
        let len = rows.len();
        Self {
            column_memory: HotMetadataCharge::new(crate::core::Schema::column_names_bytes(
                &columns,
            )),
    ...
    pub fn with_arc_columns(columns: CompactArc<Vec<String>>, rows: RowVec) -> Self {
        let len = rows.len();
        Self {
            column_memory: HotMetadataCharge::default(),
            columns,
            rows: RowStorage::Owned(rows),
            len,
            current_index: None,
...
impl QueryResult for ExecutorResult {
    ...
    fn with_aliases(
        ...
        let copied = CompactArc::strong_count(&self.columns) > 1;
        ...
        if changed {
            self.column_memory
                .resize(crate::core::Schema::column_names_bytes(columns));
        }
        self
    }
}
...
impl ColumnarResult {
    ...
    pub fn new(columns: Vec<String>, data: Vec<Vec<Value>>) -> Self {
        ...
        Self {
            _column_memory: HotMetadataCharge::new(crate::core::Schema::column_names_bytes(
                &columns,
            )),
            columns: CompactArc::new(columns),
            data,
    ...
    pub fn with_arc_columns(columns: CompactArc<Vec<String>>, data: Vec<Vec<Value>>) -> Self {
        ...
        Self {
            _column_memory: HotMetadataCharge::default(),
            columns,
            data,
            num_rows,
            current_index: None,
