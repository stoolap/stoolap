...
#[derive(Default)]
struct ExistsSchemaCache {
    entries: FxHashMap<String, CompactArc<Vec<String>>>,
    memory: HotMetadataCharge,
}

impl ExistsSchemaCache {
...
thread_local! {
    static EXISTS_SCHEMA_CACHE: RefCell<ExistsSchemaCache> = RefCell::new(ExistsSchemaCache::default());
}
...
pub fn get_cached_exists_schema(key: &str) -> Option<CompactArc<Vec<String>>> {
    EXISTS_SCHEMA_CACHE.with(|cache| cache.borrow().entries.get(key).cloned())
}
