...
impl HeapString {
    ...
    unsafe fn release(pointer: *mut Self) {
        // SAFETY: each SmartString owns one reference to this Box allocation.
        // Release/acquire orders prior owners' accesses before final destruction.
...
impl SmartString {
    ...
    pub fn into_string(self) -> String {
        ...
        // Decide whether the backing is charged only AFTER exclusive ownership
        // is acquired: a different owner can adopt and drop before this CAS.
        unsafe {
            if (*pointer)
                .strong
                ...
                .is_ok()
            {
                ...
                drop(backing);
                if charge.is_some() {
                    // Keep the moved buffer's charge alive until that buffer is
                    // destroyed. An ordinary returned String owns a fresh copy.
                    let copy = text.clone();
                    drop(text);
                    drop(charge);
                    return copy;
