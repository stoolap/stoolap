...
| `arena_capacity_bytes` | Capacity of chunk slots, metadata, directories, free lists and prepared or detached buffers |
...
| `retired_arena_payload_bytes` | Removed arena payloads awaiting destruction |
| `version_tree_bytes` | Current hot tree node capacity and previous-version links |
| `transaction_version_bytes` | Spilled transaction-local version-history capacity |
| `transaction_undo_bytes` | Index-undo buffer and key-vector capacities |
| `transaction_map_bytes` | Process-wide active and pooled transaction-map capacity, only in `*` |
| `index_requested_bytes` | Reported built-in index object and buffer capacities |
| `index_estimated_bytes` | Named bounds for opaque hash, B-tree and Roaring allocations |
| `index_scratch_bytes` | Process-wide HNSW query and build scratch capacity, only in `*` |
| `row_claim_bytes` | Row-claim map capacity, including retained empty capacity |
| `row_pool_bytes` | Process-wide cached row and row-ID buffer capacity, only in `*` |
| `transaction_registry_bytes` | Process-wide transaction-state, snapshot and isolation map capacity, only in `*` |
| `hot_metadata_bytes` | Process-wide reported catalog, schema and auxiliary buffer capacities, only in `*` |

These are partial requested-byte subtotals, excluding allocator rounding. They do not represent process RSS or a complete engine budget. Counters saturate at their platform and API limits. The existing hot-size trigger is unchanged.

Reads, exports, projections and shared-reference clones do not charge payloads. Version-payload, exported-payload and pinned-snapshot columns are removed. Payloads outside live or retiring arena slots, old tree roots, shared cache/default children and Arc backing retained only by weak references are outside these subtotals. On-demand reporting with pointer deduplication and complete retained-allocation ownership are deferred to a later change.

Arena capacity follows allocation, growth and destruction, including reserved slots and pending retirement buffers. Chunks have up to 262,144 slots and can be released independently. Current version-tree capacity includes node headers, fixed entry/child capacities and history-link allocations. Copy/merge scratch and detached roots retained by readers or pending TRUNCATE destruction are excluded.

Transaction maps retain their charges while pooled; registry and row-claim maps follow actual capacity. Transaction history and undo charges remain through savepoints and rollback until their owners release them. The row-pool subtotal covers empty cached buffers and outer pool vectors; active result buffers are excluded. Process-wide subtotals include other open database instances and must not be summed across instances.

Hot metadata includes owned schema strings, column/FK vectors and column-index maps, catalog map/key capacity, view definitions, reverse-FK lists and zone-map segment capacity. Schema mutations refresh these capacities through `schema_mut()` or engine adoption. Shared schema-name caches, PK-index caches, defaults and zone-map value children are excluded. Fresh result/compiled COUNT name allocations and EXISTS cache map/key capacity are included while their allocating owners live; copying shared names adds no charge. Prepared commit, rollback and composite-index cleanup buffers charge their capacities. Fixed object envelopes remain conservative and may include stack-created objects. No row, string or Arc header is enlarged.

Index requested bytes include metadata, reverse-map allocations, primary-key bitsets and stored vector capacities. Hash indexes include freshly allocated key Arc envelopes but exclude shared value children. B-tree and bitmap shared key owners add no charge. Composite indexes include key, row-ID and reverse-pointer vectors, including lazily allocated range/prefix structures. HNSW includes packed vectors, nodes, neighbors, reverse maps and retained scratch buffers. Reads that allocate a scratch buffer or build a lazy index structure charge its new capacity once; visiting entries does no payload accounting.

The opaque index estimates use these assumptions:

- Hash buckets: for retained capacity high-water `h`, zero if `h = 0`, otherwise `2 * h * (size_of(bucket) + 1) + 2 * max(align_of(bucket), 16)`. This assumes Swiss-table geometry with at most twice the reported capacity in buckets and 16-byte control groups. Deletion and clear preserve the high-water mark.
- B-tree nodes: `(1 + entries / 5) * 16 * (size_of(key) + size_of(value) + size_of(pointer))`. This assumes at most 11 keys per node and at least five in each non-root node. Retained empty trees include a possible root; clear releases the estimate.
- Roaring: directory capacity is bounded by `64 * S * max(4, 2 * min(H, 65536))`, where `S` is the current 32-bit submap count and `H` the container-count high-water mark. Submap nodes use the B-tree formula. Container payload allowance starts at 8 bytes, grows by 4 per successful insertion, and is capped at 8,192 bytes per surviving container. Container destruction subtracts 8 and the allowance becomes zero when no containers remain. This uses Roaring 0.11.3 growth behavior; churn can overcount. Its five tracking scalars cost 40 bytes per distinct indexed value on 64-bit builds.

Registered index owners keep their capacity charges after DROP INDEX or DROP TABLE until destruction. The `*` row includes retained registered stores and indexes, so it can exceed the sum of current table rows. Custom indexes without a memory account, transient rehash/rebuild overlap, returned search buffers and unlisted operator buffers are excluded. `volume_bytes` remains an estimate for registered volumes; pinned cold generations and full cold-ledger coverage belong to later stages.
