...
| `hot_max_rows` | Integer | 262144 | Committed hot rows per table that request an early seal (0 = off) |
| `hot_max_bytes` | Integer (bytes) | 0 | Hot row bytes per table above which commits wait for a seal (0 = off) |
