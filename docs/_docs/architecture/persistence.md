...
A table rename moves side files with their volumes; DROP TABLE, TRUNCATE and compaction retire them with their volumes, each removed once its last reader lets go. On reopen a side file is attached to its volume when it reads correctly; one that does not open is left in place and logged, and its volume is uncovered until the next compaction rewrites it.
