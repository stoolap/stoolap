...
A statement whose WHERE clause bounds an indexed column with one equality, or a lower and an upper bound, probes each volume's side file before the volume's data is touched: after the zone map and bloom filter pruning, and before a metadata-only volume is reloaded. The side file's directory decides first, without a page read or a reservation: no key page in the range skips the volume (`misses`), and an estimate of the candidates (the pages inside the range exactly, the boundary pages by key interpolation) above one twentieth of the volume's rows sends the volume through the scan (`cost_scans`), so an index the statement will not use costs it no page. A volume that one position page holds (8,190 rows) is scanned about as fast as its pages are read from the file, so it is probed only when the pages are cached or fit in the cache without an eviction, at their parsed size with the raw page of a read beside them (else `page_scans`): under a budget too small for its volumes' pages, a table of small volumes keeps the scan's cost rather than paying a file read per volume. Otherwise the probe reads the boundary key pages through a reader of its own, let go before the decision is handed on, and yields the exact candidate positions; an empty answer skips the volume without a reload, and an exact count above the same share sends the volume through the scan. The candidates are read window by window (4,096 positions), each row checked by the same rules as the scan (the visibility bitmap, the committed tombstones under the snapshot rule, the transaction's own deletions, the hot version that shadows a cold copy) and materialized with only the columns the statement needs, so a LIMIT stops after the window that satisfies it; the other predicates of the statement, a text equality among them, are checked on the candidates alone, without the dictionary pass over the whole volume the scan would make first. When the volume's turn comes the walk's reader takes one working reservation from `INDEX_CACHE_MB` and lets it go when the walk ends, so a budget that holds one reader serves the volumes of a statement in turn; refused, the volume goes through the scan (`refused`), and admitted, the reader completes on its own buffers whatever the cache admits afterwards. A TIMESTAMP bound outside the range nanoseconds hold (past the year 2262, before 1677) has no exact key: the column goes through the scan, where such a bound is left to the timestamp comparison itself rather than narrowed by nanoseconds. The ordered LIMIT collector, which the executor calls again with a larger offset for a residual predicate, sorts the walk's positions so its order is the scan's whatever the admission decides per call; the positions are admitted against `INDEX_CACHE_MB` beside the reader and charged while they live, and refused, the scan reads the same order. A closed scanner lets its walk's reader go, as a dropped one does. A side file whose column does not stand for the index's current identity (an older volume, or an index recreated since) goes through the scan (`ineligible`). `PRAGMA INDEX_READ_STATS` counts probes, misses, candidates, rows, windows, refusals, ineligible volumes, cost-decided scans, page-decided scans and reloads.

Side files are built under two process-wide budgets, set with `PRAGMA INDEX_CACHE_MB` (the pages readers hold, default 16) and `PRAGMA INDEX_BUILD_MB` (the workspaces of the builds in flight and the decode of their input, one row group at a time, default 64). A build that does not fit beside the others is refused and the volume is published without a side file, uncovered, with the refusal counted and logged; a side file that cannot be written or read back is counted apart. An error reading the volume itself fails the seal or the compaction, as any other error preparing the volume does. Lowering a budget never revokes what a reader or a build already holds. Each side file column records the identity of the index definition it was built for: the LSN of the WAL record that created the index, which the catalog keeps through checkpoint copies, replay and restore and never issues twice, so an index dropped and created again with the same definition is a new index. A seal or compaction compares its side files with the catalog under the DDL guard before it publishes: a side file none of whose columns still stands for a current index is discarded and the volume published uncovered; a column whose index changed meanwhile is simply never served, while the other columns of the same file stay covered. The seal holds the DDL guard through its hot cleanup, so index and column DDL wait for a seal's fence section. Side files of the earlier format (VERSION 3) carry no identities and count as uncovered. An uncovered volume is covered again when a compaction rewrites it, or before that by the backfill: one volume per maintenance cycle, or a whole pass on `PRAGMA INDEX_BACKFILL`, builds the side file beside the volume under half of `INDEX_BUILD_MB` while nothing else builds, so the seal always keeps its share, and attaches it under the DDL guard once it covers every current index and the volume is still registered; a file that does not is discarded, and a volume compacted away meanwhile takes its file with it. A reader holding an older view keeps scanning the volume; the next read probes it. A file that replaces a volume's side file carries its generation's name, `vol_<id>.g<generation>.sidx`, beside the plain one the seal wrote: no generation is overwritten in place, the holders of the file before keep reading it, reopen attaches the newest generation that opens and retires the rest, and every generation goes with its volume. A reader checks the column's identity against the catalog before its first row, so a file attached at reopen for an index that was since recreated sends the volume through the scan. `PRAGMA INDEX_STATS` reports the three ledgers (cache, builds, resident directories), their refusals, and the builds failed and side files discarded since the process started.

A table rename moves side files with their volumes; DROP TABLE, TRUNCATE and compaction retire them with their volumes, each removed once its last reader lets go. DROP INDEX takes out and removes the side file of every volume none of whose columns, at the positions the volume holds them, still stands for a current index; a file that still serves another index stays whole until the volume's next compaction builds it anew. A reopen makes the same decision once the log is replayed and the catalog is final, so a file a replayed DROP would have removed stays when a later CREATE stands for it. On reopen a side file is attached to its volume when it reads correctly; one that does not open is left in place and logged, and its volume is uncovered until the next compaction rewrites it.
