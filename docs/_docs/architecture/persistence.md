...
A statement whose WHERE clause bounds an indexed column with one equality, or a lower and an upper bound, probes each volume's side file before the volume's data is touched: after the zone map and bloom filter pruning, and before a metadata-only volume is reloaded. The probe reads the boundary key pages through a reader of its own, let go before the decision is handed on, and yields the exact candidate positions; an empty answer skips the volume without a reload, and a candidate count above one twentieth of the volume's rows sends the volume through the scan instead (`cost_scans`). The candidates are read window by window (4,096 positions), each row checked by the same rules as the scan (the visibility bitmap, the committed tombstones under the snapshot rule, the transaction's own deletions, the hot version that shadows a cold copy) and materialized with only the columns the statement needs, so a LIMIT stops after the window that satisfies it; the other predicates of the statement, a text equality among them, are checked on the candidates alone, without the dictionary pass over the whole volume the scan would make first. When the volume's turn comes the walk's reader takes one working reservation from `INDEX_CACHE_MB` and lets it go when the walk ends, so a budget that holds one reader serves the volumes of a statement in turn; refused, the volume goes through the scan (`refused`), and admitted, the reader completes on its own buffers whatever the cache admits afterwards. A TIMESTAMP bound outside the range nanoseconds hold (past the year 2262) has no exact key: the column goes through the scan, which saturates the bound. A side file whose column does not stand for the index's current identity (an older volume, or an index recreated since) goes through the scan (`ineligible`). `PRAGMA INDEX_READ_STATS` counts probes, misses, candidates, rows, windows, refusals, ineligible volumes, cost-decided scans and reloads.
